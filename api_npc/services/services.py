...
class NpcService:
    ...
    def update_npc(self, npc_id, npc):
        try:
...
if __name__ == '__main__':
    ...
    try:
        ...
        #Add a new NPC
        #new_npc = npc_service.add_npc({'name': 'Goblin', 'type': 'monster'})
        #logger.info(f'New NPC added: {new_npc}')
        #Get NPC by id
        #npc = npc_service.get_npc_by_id(1)
        #logger.info(f'NPC fetched by id: {npc}')
        #Update NPC
        updated_npc = npc_service.update_npc(1, {'name': 'Goblin', 'type': 'monster', 'level': 1})
